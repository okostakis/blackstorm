...
class myFL(legoPart):
    ...
    useColor='white'
    loggingLevel=3
    calcGyroAngleCorrection=False
    calcSteeringCorrection=False
    corrections=[]
    timer=myTimer()
    start=0
    i=0
    pid=myPID(loggingStats=False,loggingLevel=3)
    

    def __init__(self,robot):
        self.r=robot
        self.logger = myLogger(name='F.L. Class')
       

    def Init(self, ports=2, target=50, leftRight=-1, kp=0.1, ki=0, kd=0.3, dt=0.01, minSpeed=10, maxSpeed=25,speedRatio=1, comments="", useColor='white',calcGyroAngleCorrection=False,calcSteeringCorrection=False):
        # s = "\n=================================================================="
        # if comments!="":
        #     s+="\n| --> "+comments+" <--"
        #     s+="\n=================================================================="
        ...
        # s +=",minSpeed=" + str(minSpeed*speedRatio) + ",maxSpeed=" + str(maxSpeed*speedRatio) + ", waits=" + self.ListToStr(waits) + ",stop_action='" + stop_action + "')"
        if calcGyroAngleCorrection:
            self.calcGyroAngleCorrection=True
            self.r.gyro.values=[]
            self.r.gyro.i=0
        if calcSteeringCorrection:
            self.calcSteeringCorrection=True
            self.corrections=[]
        self.ports=ports
        self.kp=kp
        self.kd=kd
        ...
        self.timer.dt=dt
        self.pid=myPID(name=self.name+'.FollowLine (pid)',kp=kp,ki=ki,kd=kd,loggingStats=False,loggingLevel=self.loggingLevel)
        self.i=0
        self.currSpeed=minSpeed*speedRatio
        self.maxSpeed=maxSpeed*speedRatio
        self.powerStep=self.currSpeed/abs(self.currSpeed)
        self.r.motorB.ResetDegrees()
        self.r.motorC.ResetDegrees()
    ...
    def Step(self):
        ...
        self.timer.Step()
        if self.ports==1:
            input1=self.r.color1.GetLight(color=self.useColor)
            cor=self.pid.Step(inputRead=input1,target=self.target)*self.leftRight
        if self.ports==2:
            input2=self.r.color2.GetLight(color=self.useColor)
            cor=self.pid.Step(inputRead=input2,target=self.target)*self.leftRight
        elif self.ports==3:
            input3=self.r.color3.GetLight(color=self.useColor)
            cor=self.pid.Step(inputRead=input3,target=self.target)*self.leftRight
        elif self.ports==23:
            input2=self.r.color2.GetLight(color=self.useColor)
            input3=self.r.color3.GetLight(color=self.useColor)
            if self.target==0:
                cor=-self.pid.Step(inputRead=(input2-input3),target=self.target)
            else:
                cor= self.pid.Step(inputRead=input2,target=self.target)*input3/100*self.leftRight
        elif self.ports==32:
            input2=self.r.color2.GetLight(color=self.useColor)
            input3=self.r.color3.GetLight(color=self.useColor)
            if self.target==0:
                ...
                cor= self.pid.Step(inputRead=input3,target=self.target)*input2/100*self.leftRight
        elif self.ports==4:
            input1=self.r.color4.GetLight(color=self.useColor)
            cor=self.pid.Step(inputRead=input1,target=self.target)*self.leftRight
        if self.maxSpeed<0:
            cor=-cor    
            
        
        self.r.motorB.SetSpeed(speed=(self.currSpeed+cor))
        self.r.motorC.SetSpeed(speed=(self.currSpeed-cor))
        if self.calcSteeringCorrection:
            self.corrections.append(cor)
        
 
        if abs(self.currSpeed)<abs(self.maxSpeed):
            if abs(self.pid.error)<=(100-self.target)/5:
                self.currSpeed += self.powerStep
        if abs(self.currSpeed)>abs(self.maxSpeed):
            self.currSpeed -= self.powerStep*2.5

        if self.calcGyroAngleCorrection:
            self.r.gyro.GetAngle()

    def Done(self):
        totalTime=time.perf_counter()-self.start
        distB=self.r.motorB.GetDegrees()
        distC=self.r.motorC.GetDegrees()
        self.soDegrees = abs(distB + distC) / 2
        s   ="\n| dt="+str(totalTime/self.i)
        s += '\n| degrees/dt:'+str((distB+distC)/2/self.i)+' ('+str(self.r.DegreesToDistance(distB+distC)/2/self.i)+' mm/dt)'
        s += '\n| motorB:'+str(distB)+" ("+str(self.r.DegreesToDistance(distB))+" mm)"
        s += '\n| motorC:'+str(distC)+" ("+str(self.r.DegreesToDistance(distC))+" mm)"
        s += "\n| soDistance= " + str(self.soDegrees) + " (" + str(self.r.DegreesToDistance(self.soDegrees)) + " mm)"
        s += "\n=========================================================="

        self.Print(s)
        s2 ="\nFollowLine (Real) dt="+str(totalTime/self.i)
        self.Print(s2)
        ...
        if self.calcGyroAngleCorrection:
            self.r.gyro.values.sort()
            target=round(self.r.gyro.value/90)*90
            l=len(self.r.gyro.values)
            # self.Print('angle before correction',self.r.gyro.GetAngle())            
            self.r.gyro.angleCorrection=self.r.gyro.angleCorrection+target-self.r.gyro.values[l//2]
            # self.Print('angle after correction',self.r.gyro.GetAngle())
            # self.Print('angle values=',self.r.gyro.values)
            self.Print('angle correction=',self.r.gyro.angleCorrection)

        if self.calcSteeringCorrection:
            self.corrections.sort()
            l=len(self.corrections)
            self.Print('steering correction=',self.corrections[l//2])
            self.r.steeringCorrection=self.corrections[l//2]
...
class myFA(legoPart):
    ...
    def __init__(self,robot):
        self.r=robot
        self.logger = myLogger(name='F.A. Class')
        

    def Init(self, target=0, kp=1, ki=0, kd=0, dt=0.01, minSpeed=10, maxSpeed=30,speedRatio=1, comments=""):
        # s = "\n=================================================================="
        # if comments!="":
        #     s+="\n| --> "+comments+" <--"
        #     s+="\n=================================================================="
        ...
        self.target=target
        self.timer.dt=dt
        self.pid=myPID(name=self.name+'.FollowAngle (pid)',kp=kp,ki=ki,kd=kd,loggingStats=False,loggingLevel=self.loggingLevel)
        self.i=0
        self.currSpeed=minSpeed*speedRatio
        self.maxSpeed=maxSpeed*speedRatio
        self.powerStep=self.currSpeed/abs(self.currSpeed)
    ...
    def Step(self):
        ...
        self.i=self.i+1
        self.timer.Step()
        input=self.r.gyro.GetAngle()
        cor=self.pid.Step(inputRead=input,target=self.target)
        # if self.maxSpeed<0:
        #     cor=-cor    
        self.r.motorB.SetSpeed(speed=(self.currSpeed+cor))
        self.r.motorC.SetSpeed(speed=(self.currSpeed-cor))
    ...
    def Done(self):
        ...
        self.Print(s)
        s2 ="\nFollowAngle (Real) dt="+str(totalTime/self.i)
        self.Print(s2)
...
class myCar(legoPart):
    ...
    def GetLeftDistance(self,maxDistance=100,minDistance=10):
        x=self.ultraSonic1.GetDistance()
        if x<maxDistance and x>minDistance:
            self.leftDistance=x
    ...
    def GetRightDistance(self,maxDistance=100,minDistance=10):
    ...
    def Turn(self,angle=-90,turn=1,speed=30,stopAtEnd=False):
        c=self
        c.UpdateSendorData()
    ...
    def Turn3(self,turn=1,lane=1):
        c=self
        if lane==1:
            marko1=20
        else:
            marko1=24
        #ang=η γωνια που προχορα 
        ...
        while True:
            ...
            c.MoveToAngle(targetAngle=ang0+ang,speed=50)
            c.SetSonarAngle(angle=50*turn,speed=60)
            if c.fullDistance>marko:
                c.StopAllMotors()
                break
            if c.frontDistance<50 and abs(c.sonarAngle)+abs(c.angle-ang0)>80:
                lane2ob=True
                c.Beep()
        ...
        if lane2ob:
            ...
            while True:
                c.UpdateSendorDataLimited()
                c.SetSonarAngle(angle=-40,speed=40)
                c.MoveToAngle(targetAngle=ang0+40*turn,speed=50)
            ...
            while True:
                c.UpdateSendorDataLimited()
                c.MoveToAngle(targetAngle=ang0+ang*turn,speed=50)
    ...
    def UpdateSendorData(self,print=False,log=False):
        ...
        if self.sensor_i % 3 ==0:
            ...
            self.leftDistance= self.GetLeftDistance(minDistance=self.minSideDistance,maxDistance=self.maxSideDistance)
        else:    
            self.rightDistance= self.GetRightDistance(minDistance=self.minSideDistance,maxDistance=self.maxSideDistance)
    ...
    def Turn3(self,speed=50,turn=1,lane=1,log=False):
        ...
        while True:
            ...
            c.SetSonarAngle(angle=ang1+turn*15-c.angle,speed=40)
            if c.fullDistance>marko:
                c.StopAllMotors()
                break
            if c.frontDistance<60:
                if log and not lane2ob:
                    c.LogSensorData()
                if abs(ang1+turn*15-c.angle-c.sonarAngle)<10:
                    lane2ob=True
    ...
    def Turn4(self,speed=50,turn=1,lane=1,log=False):
        # cannot understand lane
        c=self
        ang0=round(c.angle/90)*90
        ang1=ang0+90*turn
        while True:
            c.MoveToAngle(speed=50,targetAngle=ang0,angleKp=2)
            c.UpdateSendorDataLimited()
            if turn==1:                
                if c.frontDistance<=48:
                    right=c.GetRightDistance()
                    c.StopAllMotors()
                    c.Print('rightDistance:',right)
                    break
                else:
                    c.StopAllMotors()
                    break
