...
for i in range(1,rounds):
    ...
    while True:
        ...
        fix=c.CalcTargetAngleFromLeftWallDistance(targetLeftDistance=30,acceptedError=5,maxCorrection=12)
        c.MoveToAngle(targetAngle=laneAngle*turn+fix*0.7,speed=50)
        c.SetSonarAngle(angle=(laneAngle*turn)-c.angle,speed=40)
        if (c.fullDistance>marko and c.rightDistance*c.leftDistance<0):
            turnFound=True
        if turnFound  and c.frontDistance<40:
            c.StopAllMotors()
            c.Beep()
            break
    # calc gyro error
