...
# end turn3
if turn==-1:
    mariposa=c.rightDistance
else:
    mariposa=c.leftDistance
...
for i in range(1,rounds):
    ...
    while True:
        c.UpdateSendorData()
        fix=c.CalcTargetAngleFromWallDistance(targetLeftDistance=25,acceptedError=5,maxCorrection=12)
        c.MoveToAngle(targetAngle=laneAngle*turn+fix,speed=60)
        c.SetSonarAngle(angle=(laneAngle*turn)-c.angle,speed=40)
        if (c.fullDistance>marko and c.frontDistance<60) and c.rightDistance*c.leftDistance<0:
            c.StopAllMotors()
            c.Beep()
            break
    ...
    l=len(angleList)
    c.Print('mo angle ',angleList[l//2])
