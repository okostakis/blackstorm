...
while True:
    ...
    c.SetSonarAngle(angle=-c.angle, speed=60)
...
for i in range(1,rounds):
    ...
    if i==1:
        marsoupilami=55
        c.Turn(angle=80*turn,speed=47)
        lane=2
    else:
        marsoupilami=0
        c.Print('lane:',lane,'front:',c.frontDistance,'fullDistance:',c.fullDistance,'markDistance:',c.markDistance)
        lane=c.Turn3(speed=50,turn=turn,lane=lane)

    c.markDistance=c.markDistance=marsoupilami
    ...
    while True:
        ...
        if c.frontDistance<=25:
            ...
            if lane==1:
                if turn==1:
                    ...
                    c.RightToLeftLane()
                c.markDistance=c.markDistance+22
                lane=2
            else:
                if turn==1:
                    c.RightToLeftLane()
        ...
        c.MoveToAngle(targetAngle=laneAngle*turn+fix*0.95,speed=55)
        # το μπροστινό sonar κοιτάει πρός τον μπροστινό τοίχο
        c.SetSonarAngle(angle=(laneAngle*turn)-c.angle,speed=55)
        # Αν στα αριστερο,δεξιο sonar βρεθεί αρνητική τιμή
        # τότε βρέθηκε κενό αριστερά ή δεξια οπότε έχω στροφή
        #(c.leftDistance>50 or c.rightDistance>50)
        if (c.fullDistance>marko )and c.frontDistance<71:
            turnFound=True
            c.Beep()
            g=i
            # c.StopAllMotors()
